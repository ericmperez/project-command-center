...
export function calculateSchedulingEstimate(
  ...
  // Calculate suggested start date working backwards from effective deadline
  let suggestedStartDate: string | null = null;
  let status: SchedulingEstimate['status'] = 'no_deadline';

  // Use the earlier of nextMeetingDate and targetCompletionDate as effective deadline
  const deadlineCandidates = [nextMeetingDate, targetCompletionDate].filter(Boolean) as string[];
  const effectiveDeadline = deadlineCandidates.length > 0
    ? deadlineCandidates.reduce((a, b) => (new Date(a) < new Date(b) ? a : b))
    : null;

  if (effectiveDeadline) {
    const meetingDate = new Date(effectiveDeadline);
